("stdint.h" "x264.h")
#.(labels ((enumf (name &rest rest)
	   `(:enum ,name ,(mapcar (lambda (x) `(,x ,(symbol-name x))) rest)))
	 (short->lisp-type (type)
	   (and (symbolp type)
		(macrolet ((p (tp)
			     `(eq type ',tp)))
		  (cond ((or (p int) (p uint8_t) (p uint32_t) (p x264_image_t))
			 type)
			((eq type 'float) 'sb-alien:single-float)
			((eq type 'uint8_t*) '(* sb-alien:unsigned-char))))))
	   (parse-declaration (type lisp-name)
	   (let ((lisp-type (short->lisp-type type))
		 (c-type (string-downcase
			  (symbol-name type)))
	     ...
	     (,(concatenate 'string
		  ...
		  (reduce
		   ...
		   (mapcar (lambda (e)
			     (cond  
			       ((eq (car e) 'structi)
				(destructuring-bind (structi name &rest decls) e
				  (loop for (type slot-name) in decls collect
					(let* ((c-name (string-downcase
							(format nil "~a.~a" 
								(symbol-name name) 
								(symbol-name slot-name))))
					       (lisp-name (intern (string-upcase c-name)))
					       (lisp-type (short->lisp-type type))
					       (c-type (string-downcase
							(symbol-name lisp-type))))
					  (when lisp-type
					    `(,lisp-type ,lisp-name ,c-type ,c-name))))))
			       ((consp (car e)) nil)
			       (t (destructuring-bind (type lisp-name) e
				    `(,(parse-declaration type lisp-name)))))) rest))))))
	 (integerf (args)
	   (loop for e in args collect 
		`(:integer ,e ,(symbol-name e)))))
  ;; define macros, so that no quoting is necessary in the definitions
  (macrolet ((integer (&rest rest)
	       `(integerf ',rest))
	     (enum (&rest rest)
	       `(enumf ,@(mapcar (lambda (x) `(quote ,x)) rest)))
	     (struct (&rest rest)
